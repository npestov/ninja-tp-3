...
#pragma warning restore 0649
		...
		private void StartSlice(BzSliceTryData sliceTryData, Action<BzSliceTryResult> callBack)
		{
			...
			for (int i = 0; i < renderers.Length; i++)
			{
				...
				//if pawn then do dead color
				if (transform.CompareTag("pawn"))
					renderer.material = DefaultSliceMaterial;
        ...
        protected virtual void GetNewObjects(out GameObject resultObjNeg, out GameObject resultObjPos)
        {
			...
			//STRT OF CHAR SLICE
			if (resultObjNeg.transform.parent.GetComponents<FallDownRb>().Length == 0)
				return;

			resultObjNeg.transform.parent.GetComponent<FallDownRb>().DropWeapon();

			//drop delay
			if (resultObjNeg.transform.parent.GetComponent<FallDownRb>().dropImmediatly)
				resultObjNeg.transform.parent.GetComponent<FallDownRb>().DropRb(3);

			var dmg = FindObjectOfType<PlayerLvlManager>().playerLvl / 9;
			if (transform.parent.name == "BOSS")
            {
				dmg = 5;
            }

			if (resultObjNeg.transform.parent.GetComponent<EnemyLvlManager>().ReduceLVL(dmg))
				resultObjNeg.transform.parent.GetComponent<FallDownRb>().DropImmediatly();
			//destory shit
			Destroy(resultObjPos.GetComponent<VisionCone>());
			Destroy(resultObjNeg.GetComponent<VisionCone>());
			Destroy(resultObjPos.GetComponent<TargetScript>());
			Destroy(resultObjNeg.GetComponent<TargetScript>());

			//check which one is on top
			Rigidbody nRb = resultObjPos.GetComponent<Rigidbody>();
			Rigidbody pRb = resultObjNeg.GetComponent<Rigidbody>();

			if (resultObjNeg.transform.position.y < resultObjPos.transform.position.y)
            {
				nRb = resultObjNeg.GetComponent<Rigidbody>();
				pRb = resultObjPos.GetComponent<Rigidbody>();
			}

			//change rb properties
			nRb.isKinematic = false;
			...
			float forceSlower = 0.22f;
			bool isBoss = false;
			if (resultObjNeg.transform.parent.GetComponents<Boss>().Length == 1)
            {
				isBoss = true;
			}

			//add forces
			nRb.AddForce(currentPlane.normal * forceSlower, ForceMode.Impulse);
			pRb.AddForce(currentPlane.flipped.normal * forceSlower, ForceMode.Impulse);

			//add drag after delay
			StartCoroutine(SlowDownSides(pRb, nRb));

			if (isBoss)
				StartCoroutine(SlowDownSides(pRb, nRb, 5));
			//END OF CHAR SLICE

		}

		private IEnumerator SlowDownSides(Rigidbody pRb, Rigidbody nRb , float slowMore = 1)
        {
			nRb.drag = 0;
			pRb.drag = 0;
			yield return new WaitForSeconds(0.4f / slowMore);
			nRb.drag = 0.4f * slowMore;
			pRb.drag = 0.4f * slowMore;
		}
